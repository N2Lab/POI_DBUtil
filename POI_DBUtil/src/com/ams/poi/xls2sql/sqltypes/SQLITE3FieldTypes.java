 ...
 * @version 1.0
 */
public class SQLITE3FieldTypes {

	// XLS > SQLITE3定義マッピング
	static HashMap type_map;
	static {
		type_map = new HashMap();
		// type_map.put(XLS type, SQLITE3 type)
		type_map.put("serial", "INTEGER PRIMARY KEY AUTOINCREMENT");
		type_map.put("SERIAL", "INTEGER PRIMARY KEY AUTOINCREMENT");
		type_map.put("Timestamp", "TEXT");
		type_map.put("Date", "TEXT");
		type_map.put("date", "TEXT");
		type_map.put("Time", "TEXT");
		type_map.put("time", "TEXT");
		type_map.put("VARCHAR PK", "TEXT PRIMARY KEY");
		type_map.put("VARCHAR", "TEXT");
		type_map.put("Varchar", "TEXT");
		type_map.put("varchar", "TEXT");
		...
		type_map.put("Int", "INTEGER");
		type_map.put("CHAR", "TEXT");
		type_map.put("Char", "TEXT");
		type_map.put("char", "TEXT");
		type_map.put("RAW", "BLOB");
		type_map.put("Raw", "BLOB");
		type_map.put("raw", "BLOB");

		type_map.put("TINYINT", "INTEGER"); // not exist in SQLITE3
		type_map.put("SMALLINT", "INTEGER");
		type_map.put("MEDIUMINT", "INTEGER"); // not exist in SQLITE3
		type_map.put("BIGINT", "INTEGER");
		type_map.put("FLOAT", "REAL");
		type_map.put("DOUBLE", "REAL");
		//type_map.put("DOUBLE PRECISION", "DOUBLE PRECISION"); not exist in SQLITE3
		type_map.put("REAL", "REAL");
		type_map.put("DECIMAL", "REAL");

		//type_map.put("SET", "SET");
		//type_map.put("ENUM", "ENUM");
		type_map.put("CLOB", "TEXT");
	    ...
	    type_map.put("NUMBER", "LONG");
	    type_map.put("NUMBER", "Long");
    	// auto number
		type_map.put("INTEGER PRIMARY KEY AUTOINCREMENT", "INTEGER PRIMARY KEY AUTOINCREMENT");
...
//		type_map.put("System", "");
	...
	/**
	 * getTypeByXLS<BR>
	 * XLS定義TypeからSQLITE3用FieldTypeを取得する
	...
	public static String getTypeByXLS(String xls_type) {
		...
		if (n1 == -1 || n2 == -1 || n1 > n2) {
			...
			if (type_map.containsKey(type_only)) {
				// sqlite3はサイズ指定が無いので指定無しで出力
				result = (String)type_map.get(type_only);
//				result = (String)type_map.get(type_only) + xls_type.substring(n1, n2 + 1);
	...
	// XLS > SQLITE3 insert 値作成時に　値を日付として変換するXLSの型
    ...
    static {
        ...
        datetime_type.add("DATETIM`E");
    }

    // XLS > SQLITE3 insert 値作成時に 値を '' で囲むXLSの型(文字列の型)
		...
		if (val instanceof String) {
			...
			if (insert_type_map.containsKey(val_t)) {
			    ...
			    if (datetime_type.contains(def.getTypeXls().toUpperCase())) {
			    	// date type and value is empty, set system time 
			    	if ("NULL".equalsIgnoreCase(val_t) || val_t == null || val_t.trim().length() == 0) {
				...
				// SQLite3 では全値に''をつけてみる。ただし NULL  以外
				
				res = "NULL".equals(res) ? res :   "'" + res + "'";
...
//			System.err.println("res is " + res + ", type=" + res.getClass());
//		}
		return res;
    }
}
